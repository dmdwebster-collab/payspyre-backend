...
#: NSF last. Purely descriptive today — accruals are generated by the schedule
#: builder, which has no configurable ordering.
...
class AllocationPriorityConfig(BaseModel):
    ...
    accrual: list[str] = Field(default_factory=_default_accrual_priority)
    repayment: list[str] = Field(default_factory=_default_repayment_priority)
...
def assert_allocation_consistent_with_engine(cfg: ProductPolicyConfig) -> None:
    """Refuse a repayment ordering the allocation engine will not honour.

    The engine's regular-payment allocator is not configurable: interest →
    principal → scheduled fees, with the add-on (NSF) bucket reachable only by
    an explicit add-on payment. A product must not be able to *advertise* an
    ordering that puts principal before interest, or NSF anywhere but last,
    because the dollars would not land where the config claims. Reordering
    origination vs administration IS allowed — they share one engine bucket, so
    the swap is a no-op the engine can honour trivially.

    Accrual priority is descriptive only (accruals come from the schedule
    builder) and is therefore not constrained here.
    """

...
* migration 048's seed data (loaded by file path — migrations aren't a package):
...
from app.services.decision_reasons import CODE_RE, validate_reason_fields
...
# ---------------------------------------------------------------------------
# Migration 048 seed integrity
# ---------------------------------------------------------------------------


def test_seed_rows_all_pass_validation():
    for kind, seeds in (("reject", MIG.REJECT_REASON_SEEDS), ("cancel", MIG.CANCEL_REASON_SEEDS)):
...
def test_seed_codes_unique_per_kind():
    for seeds in (MIG.REJECT_REASON_SEEDS, MIG.CANCEL_REASON_SEEDS):
...
def test_engine_decline_codes_are_seeded_as_reject_reasons():
    ...
    seeded = {c for c, _, _ in MIG.REJECT_REASON_SEEDS}
    for engine_code in (
        REASON_BUREAU_BELOW_MINIMUM,
        REASON_ACTIVE_BANKRUPTCY,
        REASON_BANKRUPTCY_DISCHARGE_RECENT,
...
def test_seed_wording_matches_adverse_action_builtin_wording():
    ...
    the directory."""
    for code, _, text in MIG.REJECT_REASON_SEEDS:
...
def test_cancel_seeds_cover_turnkey_directory():
    seeded = {c for c, _, _ in MIG.CANCEL_REASON_SEEDS}
    assert {

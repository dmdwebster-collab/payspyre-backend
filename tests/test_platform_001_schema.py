...
This is PR P1 — the platform foundation for PaySpyre v2.
"""
import pytest
...
class TestPlatformPatientsSchema:
    ...
    def test_platform_patients_table_exists(self, db):
    ...
    def test_platform_patients_has_required_columns(self, db):
    ...
    def test_platform_patients_has_unique_indexes(self, db):
    ...
    def test_platform_patients_enum_types_exist(self, db):
...
class TestPlatformPatientFieldsSchema:
    ...
    def test_platform_patient_fields_table_exists(self, db):
    ...
    def test_platform_patient_fields_has_source_columns(self, db):
...
class TestPlatformCreditProductsSchema:
    ...
    def test_platform_credit_products_table_exists(self, db):
    ...
    def test_platform_credit_products_has_verification_matrix(self, db):
    ...
    def test_platform_credit_products_code_unique(self, db):
...
class TestPlatformCreditApplicationsSchema:
    ...
    def test_platform_credit_applications_table_exists(self, db):
    ...
    def test_platform_credit_applications_has_flow_state(self, db):
    ...
    def test_platform_credit_applications_co_applicant_fk(self, db):
...
class TestPlatformVerificationsSchema:
    ...
    def test_platform_verifications_table_exists(self, db):
    ...
    def test_platform_verifications_has_vendor_session_ref(self, db):
...
class TestPlatformConsentsSchema:
    ...
    def test_platform_consents_table_exists(self, db):
    ...
    def test_platform_consents_has_immutable_text_columns(self, db):
    ...
    def test_platform_consents_purpose_enum_exists(self, db):
...
class TestPlatformEventsSchema:
    ...
    def test_platform_events_table_exists(self, db):
    ...
    def test_platform_events_has_required_columns(self, db):
    ...
    def test_platform_events_has_indexes(self, db):
    ...
    def test_platform_events_worm_constraint(self, db):
        """platform_events should have WORM protection (UPDATE/DELETE blocked)"""
        # Create a test event
        test_event_id = db.execute(text("""
        ...
        with pytest.raises(ProgrammingError, match="Cannot modify platform_events.*append-only"):
            db.execute(text("""
        ...
        new_event_id = db.execute(text("""
            INSERT INTO platform_events (event_type, actor, payload)
            VALUES ('test_event_2', 'system', '{"test": true}')
            RETURNING id
        """)).scalar()
        db.commit()
        assert new_event_id is not None, "INSERT should still work on platform_events"

    def test_platform_events_trigger_exists(self, db):
...
class TestPlatformRelationships:
    ...
    def test_patient_fields_fk_to_patients(self, db):
    ...
    def test_application_fk_to_patients_and_products(self, db):
    ...
    def test_verifications_fk_to_patients_and_applications(self, db):
    ...
    def test_consents_fk_to_patients_and_applications(self, db):
    ...
    def test_verifications_fk_to_consents(self, db):
...
class TestPlatformDataIntegrity:
    ...
    def test_platform_patients_soft_delete_enables_duplicate_email(self, db):
        """Soft-deleted patients should allow email reuse"""
        # Create first patient
        patient_id_1 = db.execute(text("""
    ...
    def test_platform_patients_prevents_duplicate_email_for_active(self, db):
        """Active patients should not allow duplicate emails"""
        # Create first patient
        db.execute(text("""
            INSERT INTO platform_patients (email, legal_first_name, legal_last_name)
            VALUES ('duplicate@example.com', 'John', 'Doe')
        """))
        db.commit()
        ...
        with pytest.raises(IntegrityError):
            db.execute(text("""
                INSERT INTO platform_patients (email, legal_first_name, legal_last_name)
                VALUES ('duplicate@example.com', 'Jane', 'Smith')
            """))
            db.commit()
